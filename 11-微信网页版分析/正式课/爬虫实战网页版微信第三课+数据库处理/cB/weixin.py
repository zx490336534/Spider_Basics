...
class WeiXinRobot(object):
    ...
    def sync_check(self, host=''):
        ...
        try:
            ...
            matches = re.search(r'selector:"(\d+)"', text)
    ...
    def webwxsync(self):
        ...
        try:
            ...
            r = self.s.post(url, json=data, headers=self.DEFAULT_HEADERS)
            r.encoding = 'UTF-8'
            text = r.text
            json_data = json.loads(text)
            ...
            if json_data['BaseResponse']['Ret'] == 0:  # 更新同步键
                ...
                for msg in AddMsgList:
                    print('新增信息，from：%s， to：%s， 类型为：%s， 内容为：%s' % (msg['FromUserName'], msg['ToUserName'], msg['MsgType'], msg['Content']))
            return json_data
        except Exception as e:
            if text:
                save_error_to_log(text)
